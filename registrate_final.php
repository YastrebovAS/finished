...
<?php
    ...
    if(isset($_POST['register'])){
        ...
        if($username =='admin' or $email=='admin@mail.ru' or $password=='adminpassword'){
            echo('<h3>Эти данные зарегистрированы за администратором</h3>');
        }
        else
        {
        ...
        $existigs_red = $connection->prepare("SELECT * FROM redactor WHERE passwd=:password OR username=:u OR email=:t");
        ...
        $existigs_corr = $connection->prepare("SELECT * FROM evaluator WHERE passwd=:password OR username=:u OR email=:t");
        ...
        if($spec_sum > 0){
            ...
            $inserter = $connection->prepare("INSERT INTO author(username,email,pas,reg_date) VALUES (:username,:email,:password,:date)");
