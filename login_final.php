...
if(isset($_COOKIE['remember_u']) and isset($_POST['auth2'])){
    ...
    if($_COOKIE['remember_w']=='red'){
        ...
        header("Location:page_ev_final.php");
    }
...
<?php
    ...
    if(isset($_POST['auth'])){
        ...
        if($_POST['username']=='admin' and $_POST['email']=='admin@mail.ru' and $_POST['password']=='adminpassword'){
            ...
            $connection->exec('LOCK TABLES author WRITE, evaluator WRITE, redactor WRITE');
            $username = $_POST['username'];
        $email = $_POST['email'];
        ...
        $existigs_red = $connection->prepare("SELECT * FROM evaluator WHERE passwd=:password and username=:u");
        ...
        $existigs_corr = $connection->prepare("SELECT * FROM redactor WHERE passwd=:password and username=:u");
